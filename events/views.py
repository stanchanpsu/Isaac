...
def list_events(request):
	...
	outreach_list = OutreachTrip.objects.filter(date__gte=datetime.date(current_year,current_month,current_day)).order_by('date') #filter only dates past today's date
...
def register(request):
	if request.is_ajax():
		event = Event.objects.get_subclass(id = request.session['event_id'])

		registered = request.session['event_registered']
		
		# send a boolean indicating if event an ENGR class
		if hasattr(event, 'veterans'):
			is_class = True
		else:
			is_class = False
		...
		response = json.dumps({"registered":registered, "EAs_needed":event.EAs_needed, "me": str(ea), "me-id":request.user.id, "me-url": ea.picture.url, "is_class":is_class})
